...
Write-Output "Github response: ${GITHUB_RESPONSE}"
$VSCODIUM_ASSETS= $GITHUB_RESPONSE | jq '.assets'
Write-Output "VSCodium assets: ${VSCODIUM_ASSETS}"
...
if (!$VSCODIUM_ASSETS) {
  Write-Output "Release assets do not exist at all, continuing build"
...
if (!$SYSTEM_SETUP) {
  Write-Output "Building on Windows $env:BUILDARCH because we have no system-setup.exe";
  $env:SHOULD_BUILD = 'yes'
}
elseif (!$USER_SETUP) {
  Write-Output "Building on Windows $env:BUILDARCH because we have no user-setup.exe";
  $env:SHOULD_BUILD = 'yes'
}
elseif (!$WINDOWS_ZIP) {
  Write-Output "Building on Windows $env:BUILDARCH because we have no ZIP";
  $env:SHOULD_BUILD = 'yes'
}
else {
  Write-Output "Already have all the Windows $env:BUILDARCH builds"
}
